...
var (
	...
	roInitialize           = combase.MustFindProc("RoInitialize")
	roGetActivationFactory = combase.MustFindProc("RoGetActivationFactory")
	windowsCreateString    = combase.MustFindProc("WindowsCreateString")
	windowsDeleteString    = combase.MustFindProc("WindowsDeleteString")
...
func activationFactory(className string, iid *guid) (unsafe.Pointer, error) {
	...
	r, _, _ := roGetActivationFactory.Call(
		hstr,
...
func createHString(s string) (uintptr, error) {
	...
	var hstr uintptr
	r, _, _ := windowsCreateString.Call(
...
func deleteHString(h uintptr) {
	if h != 0 {
		windowsDeleteString.Call(h)
	}
}

func isWindowMinimized(hwnd uintptr) bool {
	r, _, _ := isIconic.Call(hwnd)
	return r != 0
}
...
func restoreWindowForCapture(hwnd uintptr) {
	...
	setWindowPos.Call(hwnd, 1 /*HWND_BOTTOM*/, 0, 0, 0, 0,
		uintptr(swpNoMove|swpNoSize|swpNoActivate))
}

func roInitializeApartment() error {
	r, _, _ := roInitialize.Call(uintptr(roInitMultiThreaded))
