module github.com/pidgy/unitehud

go 1.16

require (
	gioui.org v0.0.0-20220113184748-9209fd91431f
	github.com/chromedp/chromedp v0.8.5 // indirect
	github.com/disintegration/gift v1.2.1
	github.com/dreygur/webdriver v0.0.0-20220609192912-5bfa84f4ec93 // indirect
	github.com/gorilla/websocket v1.5.0 // indirect
	github.com/guptarohit/asciigraph v0.5.5 // indirect
	github.com/klauspost/compress v1.15.6 // indirect
	github.com/micmonay/keybd_event v1.1.1 // indirect
	github.com/nfnt/resize v0.0.0-20180221191011-83c6a9932646 // indirect
	github.com/ninetwentyfour/go-wkhtmltoimage v0.0.0-20150201222019-3ccfacb98ac2 // indirect
	github.com/olekukonko/tablewriter v0.0.5
	github.com/rs/zerolog v1.25.0
	github.com/skratchdot/open-golang v0.0.0-20200116055534-eef842397966
	gocv.io/x/gocv v0.28.0
	golang.org/x/image v0.0.0-20211028202545-6944b10bf410
	golang.org/x/net v0.0.0-20211216030914-fe4d6282115f
	golang.org/x/sys v0.0.0-20220825204002-c680a09ffe64
	nhooyr.io/websocket v1.8.7 // indirect
)
