...
func Defeated() {
	...
	for ; ; sleep(time.Second) {
		...
		if err != nil {
			notify.Error("🔍  Failed to capture area (%v)", err)
			continue
		}
	...
	for ; ; sleep(team.Energy.Delay) {
		...
		*/
		...
		if last == nil || last.Value != points {
			...
			if err != nil {
				notify.Warn("🔍  [Self] Failed to identify (%v)", err)
			}
...
func PressButtonToScore() {
	for ; ; sleep(time.Millisecond * 500) {
		...
		if err != nil {
			notify.Error("🔍  [%s][Self] Failed to capture energy area (%v)", server.Clock(), err)
			continue
		}
...
func Preview() {
	...
	preview := func() {
		...
		if err != nil {
			notify.Error("🔍  [%s] Failed to capture preview (%v)", server.Clock(), err)
			return
		}
		notify.Preview = img
...
func energyScoredConfirm(before, after int, at time.Time) {
	...
	if p != nil && !p.Verified {
		...
		notify.Warn(
			"🔍  [%s][Self] Failed to score because the score option was not present within 5s (-%d)",
			server.Clock(),
			before,
		)
